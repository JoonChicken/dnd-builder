...
@export var floor_height_fudge := 0.001
...
var corrected_vertices : Array


func get_num_verts() -> int:
    return base_vertices.size()


func push_vert2(vert : Vector2) -> void:
    base_vertices.append(vert)
    if base_vertices.size() > 2:
        regenerate()
    
    
func push_vert3(vert : Vector3) -> void:
    base_vertices.append(Vector2(vert.x, vert.z))
    if base_vertices.size() > 2:
        regenerate()
    
    
func pop_vertex() -> Vector2:
    return base_vertices.pop_back()
    if base_vertices.size() > 2:
        regenerate()
    else:
        clear_visual()
...
func regenerate() -> void:
    ...
    # first copy the vertices array
    corrected_vertices = base_vertices.duplicate()
    # get midpoint of line between first two vertices; direction vector will be important later
    ...
    if is_left_inner and not is_right_inner:
        corrected_vertices.reverse()
    elif not(not is_left_inner and is_right_inner):
    ...
    for i in range(corrected_vertices.size() + 1):
        var vertex : Vector2 = corrected_vertices[i % corrected_vertices.size()]
        ...
        if i != 0:
            ...
            st.add_vertex(Vector3(vertex.x, floor_height + floor_height_fudge, vertex.y)) # bottom left of quad
            st.add_vertex(next_tri_end)
        if i != corrected_vertices.size():
            next_tri_end = Vector3(vertex.x, floor_height + floor_height_fudge, vertex.y) # bottom right of quad
            st.add_vertex(next_tri_end)
    ...
    if not Geometry2D.is_point_in_polygon(centerpoint, corrected_vertices):
    ...
    var vertices_floor := [Vector3(centerpoint.x, floor_height + floor_height_fudge, centerpoint.y)]
    var vertices_ceil := [Vector3(centerpoint.x, ceil_height, centerpoint.y)]
    for i in range(corrected_vertices.size() + 1):
        var bottom_vertex : Vector2 = corrected_vertices[i % corrected_vertices.size()]
        var top_vertex : Vector2 = corrected_vertices[(corrected_vertices.size() - i) % corrected_vertices.size()]
        vertices_floor.append(Vector3(bottom_vertex.x, floor_height + floor_height_fudge, bottom_vertex.y))
...
func select() -> void:
    selected = true
    mesh_node.set_layer_mask_value(6, true)


func deselect() -> void:
    selected = false
    mesh_node.set_layer_mask_value(6, false)
    

func _on_input_event(_camera: Node, event: InputEvent, _event_position: Vector3, _normal: Vector3, _shape_idx: int) -> void:
    if event is InputEventMouseButton:
        if event.pressed and event.button_index == MouseButton.MOUSE_BUTTON_LEFT:
            print("sector click detected")
            clicked.emit(self)
